...
class WordpressSearch extends Wordpress {
  static const searchApiPath = 'wp-content/plugins/elasticpress-custom/proxy/proxy.php';

  WordpressSearch({required String wordpressDomain}) : super(wordpressDomain: wordpressDomain);
    ...
    for (var response in resultResponse["responses"]) {
      for (var hit in response["hits"]["hits"]) {
        ...
        if (source["post_type"] == "series" || source["post_type"] == "post") {
          final result = SearchResult._fromPost(CustomEndpointPost.fromJson(source));
...
class SearchResult {
  ...
  factory SearchResult._fromPost(CustomEndpointPost post) =>
      SearchResult(
          id: post.id.toString(),
          //TODO: Account for neither post nor series?
          contentType: post.isPost
              ? ContentType.media
              : ContentType.section
      );
}